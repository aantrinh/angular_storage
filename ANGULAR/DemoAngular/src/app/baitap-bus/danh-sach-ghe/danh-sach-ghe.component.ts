...
export class DanhSachGheComponent implements OnInit {
  ...
  mangGhe = [
    ...
    { SoGhe: 19, TenGhe: "số 19", Gia: 100, TrangThai: true },
  ...
  //tao mang luu lai so ghe duoc chon vao 1 mang de xu ly
  danhSachGheDangChon = [];
  ...
  chonGhe(objGhe) {
    console.log(objGhe)
    if (objGhe.trangThaiGhe) {
      // them GHE
      this.danhSachGheDangChon.push(objGhe.ghe);
    } else {
      // xoa GHE
      /**
       *0. tim vi tri => findIndex
       *1. xoa phan tu trong mang danhSachGheDangChon => splice(viTri, 1)
       */

      let index = this.danhSachGheDangChon.findIndex(item => item.SoGhe === objGhe.ghe.SoGhe);
  ...
  huyGhe(index, SoGhe){
      ...
      this.tagListGhe.map(item =>{
        // tien hanh kiem tra so ghe dang chon
